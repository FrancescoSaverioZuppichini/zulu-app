...
export async function saveUserChat(userId: string, contactId: string, messages: MyUIMessage[]) {
    ...
    const userChats = await redis.lrange<ChatPreview>(userChatsKey, 0, -1);
    const chatIndex = userChats.findIndex(c => c.contactId === contactId);
    ...
    if (chatIndex !== -1) {
        ...
        if (lastMessage) {
            ...
            if (lastPart && lastPart.type === 'text') {
                userChats[chatIndex].lastMessage = lastPart.text;
                await redis.del(userChatsKey);
                if (userChats.length > 0) {
                    await redis.lpush(userChatsKey, ...userChats.map(c => JSON.stringify(c)));
                }
            }
        }
    }
